...
const refreshToken = async (req, res) => {
    try {
        ...
        try {
            ...
            return res.json({ msg: 'Token ativo!', user: { id } })
        } catch (error) {
            console.log(error)
            if (error.name === 'TokenExpiredError') {
                ...
                if (!session) {
                    ...
                    return res.status(401).json({
                        msg: 'Sessão não encontrada, faça o login novamente!', code: "logout"
                    })
                }
                ...
                if (session.createdAt < now) {
                    ...
                    res.clearCookie('token', { httpOnly: true, sameSite: 'None', secure: true })
                    return res.status(401).json({ msg: 'Sessão expirada, faça o login novamente!', code: "logout" })
                }
                ...
                if (!userFound) {
                    return res.status(500).json({ msg: 'Usuário da sessão não encontrado!', code: "logout" })
                }
            ...
            return res.status(401).json({ msg: 'Token Inválido.', code: 'logout' })
        }
    } catch (error) {
        console.log(error)
        return res.status(500).json({
            msg: 'Opsss erro no servidor, tente novamente!'
        })
    }
}
